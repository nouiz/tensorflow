...
ReductionCodegenInfo IrEmitterUnnested::ComputeReductionCodegenInfo(
  ...
  int smallest_input_dtype_bits = primitive_util::BitWidth(
      first_reduce->operand(0)->shape().element_type());
  for (auto input : unnested_hlo->operands()) {
    auto input_dtype_bits =
        primitive_util::BitWidth(input->shape().element_type());
    smallest_input_dtype_bits =
        std::min(input_dtype_bits, smallest_input_dtype_bits);
  }
